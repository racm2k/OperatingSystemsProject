...
int main(int argc, char const *argv[]) {
	...
	for (int i = 0; i < NCHILDS; i++)
	{
		...
		else if (pid[i] == 0)
		{
			...
			while (g != NULL) {
				...
				send_mutation_array(mutation_array,fds, x, 0);
	...
	while((c=read(fds[0],super_string, 4096))!=0){
...
GENE_DICT * find_gene_dict (char * name) {
	...
	void send_mutation_array (MUTATION_ARRAY * mutation_array, int *fds, int getpid, int detail) {
		char * buf = (char *) malloc(sizeof(char) * 1000000);
		for (int i=0 ; i<mutation_array->n_mutations ; i++) {
			MUTATION * aux = mutation_array->mutations + i;
